...
def encode_images(pixels: torch.Tensor, vae: torch.nn.Module, weight_dtype):
    # Convert 1-channel grayscale to 3-channel if needed
    if pixels.ndim == 4 and pixels.shape[1] == 1:  # [B, 1, H, W]
        pixels = pixels.expand(-1, 3, -1, -1)  # More efficient than repeat()
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        required=False,
        help=("(Optional) HuggingFace dataset name/path"),
    )
    ...
    ### NEWLY ADDED ARGUMENTS FOR OUR MANGA DATASET

    parser.add_argument(
    "--manga_dataset_dir",
    type=str,
    default=None,
    help="Path to directory containing paired manga images (BW and color versions)",
    )
    ...
    type=str,
    default="bw_image_",
    help="Filename prefix for black-and-white manga images",
    )

    parser.add_argument(
    "--color_prefix",
    type=str,
    default="color_image_",
    help="Filename prefix for colored manga images",
    )

    ###END OF THE NEW ARGUMENTS....BACK TO GENERIC CODE.
    ...
    if args.dataset_name is None and args.jsonl_for_train is None and args.manga_dataset_dir is None:
        raise ValueError("Specify one of: `--dataset_name`, `--jsonl_for_train`, or `--manga_dataset_dir`")

    if sum([args.dataset_name is not None, args.jsonl_for_train is not None, args.manga_dataset_dir is not None]) > 1:
        raise ValueError("Specify only one data source: HuggingFace dataset, JSONL file, or manga directory")
...
def get_manga_dataset(args, accelerator):
    """Create dataset from local manga image pairs"""
    bw_images = []
    color_images = []

    # Load and pair images
    for img_file in sorted(os.listdir(args.manga_dataset_dir)):
        if img_file.startswith(args.bw_prefix):
            color_file = img_file.replace(args.bw_prefix, args.color_prefix)
            if os.path.exists(os.path.join(args.manga_dataset_dir, color_file)):
                bw_images.append(img_file)
                color_images.append(color_file)

    # Create dictionary dataset format
    dataset_dict = {
        "bw_image": [os.path.join(args.manga_dataset_dir, f) for f in bw_images],
        "color_image": [os.path.join(args.manga_dataset_dir, f) for f in color_images],
        "caption": [""] * len(bw_images)  # Empty captions for colorization
    }
    ...
    return prepare_manga_dataset(dataset, args, accelerator)

def prepare_manga_dataset(dataset, args, accelerator):
    # BW transforms (will be converted to 3-channel later)
    bw_transforms = transforms.Compose([
        transforms.Resize(args.resolution, interpolation=transforms.InterpolationMode.BILINEAR),
        transforms.ToTensor(),
        transforms.Normalize([0.5], [0.5])  # Single channel norm
    ])
    ...
    def preprocess(examples):
        # Process BW images (keep as 1-channel until VAE)
        bw_images = [Image.open(img).convert("L") for img in examples["bw_image"]]
        examples["conditioning_pixel_values"] = [bw_transforms(img) for img in bw_images]

        # Process color images
        color_images = [Image.open(img).convert("RGB") for img in examples["color_image"]]
        examples["pixel_values"] = [color_transforms(img) for img in color_images]
...
def prepare_standard_dataset(dataset, args, accelerator):
    """Original dataset preparation for non-manga datasets"""
    ...
    def preprocess_train(examples):
        images = [Image.open(img).convert("RGB") if isinstance(img, str) else img.convert("RGB")
                 for img in examples[args.image_column]]
        examples["pixel_values"] = [image_transforms(img) for img in images]

        conditioning_images = [Image.open(img).convert("RGB") if isinstance(img, str) else img.convert("RGB")
                             for img in examples[args.conditioning_image_column]]
        examples["conditioning_pixel_values"] = [image_transforms(img) for img in conditioning_images]

        examples["captions"] = examples[args.caption_column]
        return examples
...
def collate_fn(examples):
    # Handle color images (pixel_values)
    pixel_values = torch.stack([example["pixel_values"] for example in examples])
    pixel_values = pixel_values.to(memory_format=torch.contiguous_format).float()

    # Handle BW images (conditioning_pixel_values)
    conditioning_pixel_values = torch.stack([example["conditioning_pixel_values"] for example in examples])

    # Ensure BW images have proper shape [B, 1, H, W] if they came in as [B, H, W]
    if conditioning_pixel_values.ndim == 3:
        conditioning_pixel_values = conditioning_pixel_values.unsqueeze(1)

    conditioning_pixel_values = conditioning_pixel_values.to(memory_format=torch.contiguous_format).float()

    # Handle captions (empty strings for colorization)
    captions = [example.get("captions", "") for example in examples]  # Default to empty string if missing
...
def main(args):
    ...
    train_dataset = get_manga_dataset(args, accelerator)
    train_dataset = prepare_manga_dataset(train_dataset, args, accelerator)

    if accelerator.is_main_process and args.manga_dataset_dir:
      sample = next(iter(train_dataset))  # Get first sample
      logger.info(f"Dataset sample verification - BW shape: {sample['conditioning_pixel_values'].shape}, "
                  f"Color shape: {sample['pixel_values'].shape}")
    ...
    logger.info(f"  Task type = {'Manga Colorization' if args.manga_dataset_dir else 'General'}")
    logger.info(f"  Num BW→Color pairs = {len(train_dataset)}")
    logger.info(f"  Input resolution = {args.resolution}")
    logger.info(f"  BW channels = {1}, Color channels = {3}")  # Add channel info
    global_step = 0
    first_epoch = 0
    ...
    if accelerator.is_main_process and args.report_to == "wandb" and args.log_dataset_samples:
        logger.info("Logging manga dataset samples.")

        # Create paired display of BW→Color images
        sample_pairs = []
        for i, batch in enumerate(train_dataloader):
            # Denormalize images
            bw_images = (batch["conditioning_pixel_values"] + 1) / 2  # [B,1,H,W]
            color_images = (batch["pixel_values"] + 1) / 2  # [B,3,H,W]

            # Convert to WandB format
            for bw, color in zip(bw_images, color_images):
                sample_pairs.append({
                    "BW_Input": wandb.Image(bw.squeeze().cpu(), caption="Black & White Input"),
                    "Color_Target": wandb.Image(color.permute(1,2,0).cpu(), caption="Color Target")
                })

            if len(sample_pairs) >= 4:  # Show 4 representative pairs
                break

        # Log as a paired comparison table
        wandb_tracker = next((t for t in accelerator.trackers if t.name == "wandb"), None)
        if wandb_tracker:
            wandb_tracker.log({
                "Manga Dataset Samples": wandb.Table(
                    columns=["BW Input", "Color Target"],
                    data=[[pair["BW_Input"], pair["Color_Target"]] for pair in sample_pairs]
                )
            })
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(flux_transformer):
                ...
                # vae encode
                bw_images = batch["conditioning_pixel_values"]
                color_images = batch["pixel_values"]
                pixel_latents = encode_images(color_images, vae.to(accelerator.device), weight_dtype)
                control_latents = encode_images(
                    bw_images,
                    vae.to(accelerator.device),
                    weight_dtype
                )
                if args.manga_dataset_dir:
                  batch["captions"] = [""] * len(batch["captions"])
                  args.proportion_empty_prompts = 1.0  # Always use empty prompt
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    ...
                    if args.validation_prompt is not None and global_step % args.validation_steps == 0:
                        # For manga, use BW images as validation inputs
                        if args.manga_dataset_dir:
